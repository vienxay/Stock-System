...
interface PRLoad {
  id: number; department?: string; purpose?: string; priority: string;
  requiredDate?: string; note?: string;
  items: {
    productId: number; quantity: number; unitPrice: number;
    supplierId?: number | null; note?: string | null;
    product: { id: number };
  }[];
}
...
export default function EditPRPage() {
  ...
  const { register, control, handleSubmit, watch, reset, formState: { errors } } = useForm<FormFields>({
    defaultValues: {
      priority: 'normal',
  ...
  const pr = (prData?.data as { data: PRLoad } | undefined)?.data;
  ...
  useEffect(() => {
    ...
    reset({
      ...
      purpose:       pr.purpose ?? '',
      priority:      (pr.priority as FormFields['priority']) ?? 'normal',
      required_date: pr.requiredDate ? String(pr.requiredDate).slice(0, 10) : '',
      note:          pr.note ?? '',
      items: pr.items.map((i) => ({
        product_id:  String(i.productId),
        quantity:    String(i.quantity),
        unit_price:  String(Number(i.unitPrice)),
        supplier_id: i.supplierId ? String(i.supplierId) : '',
        note:        i.note ?? '',
      })),
    });
  }, [pr, reset]);
  ...
  if (loadingPr) {
    return <div className="flex items-center justify-center py-20 text-gray-400">ກຳລັງໂຫຼດ...</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate(`/purchase-requests/${id}`)}
          className="p-2 rounded-lg hover:bg-gray-100 transition-colors">
          <ArrowLeft className="w-5 h-5 text-gray-600" />
        </button>
        <div>
          <h2 className="text-lg font-bold text-gray-900">ແກ້ໄຂໃບຂໍຊື້ (PR)</h2>
          <p className="text-sm text-gray-500">ແກ້ໄຂຂໍ້ມູນ ແລ້ວສົ່ງອະນຸມັດໃໝ່ (ຖ້າຖືກປະຕິເສດ)</p>
        </div>
      </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="secondary" type="button" onClick={() => navigate(`/purchase-requests/${id}`)}>
                ຍົກເລີກ
              </Button>
              <Button type="submit" loading={mut.isPending}>
                <Save className="w-4 h-4" />ບັນທຶກການແກ້ໄຂ
              </Button>
            </div>
          </div>
        </div>
